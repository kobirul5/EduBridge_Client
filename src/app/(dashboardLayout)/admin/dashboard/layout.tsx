...
export default async function Layout({ children }: DashboardLayoutProps) {
  ...
  return (
    <div className="min-h-screen bg-[linear-gradient(180deg,var(--background)_0%,var(--secondary)_48%,var(--muted)_100%)] text-foreground">
      <div className="grid min-h-screen lg:grid-cols-[290px_minmax(0,1fr)]">
        <AdminDashboardSidebar user={user} />

        <div className="relative z-0 min-w-0">
          <header className="sticky top-0 z-50 flex flex-col gap-4 border-b border-border/60 bg-card/90 px-5 py-5 backdrop-blur-md sm:px-8 lg:flex-row lg:items-center lg:justify-between">
            <div>
            ...
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-2 rounded-[1.5rem] bg-card/90 px-4 py-3 shadow-[0_18px_45px_-35px_color-mix(in_oklch,var(--primary)_35%,transparent)]">
          ...
          <main className="relative z-0 px-5 py-6 sm:px-8">
            <div className="rounded-[2rem] border border-border/70 bg-card/72 p-5 shadow-[0_24px_60px_-45px_color-mix(in_oklch,var(--primary)_45%,transparent)] backdrop-blur-md sm:p-6">
              {children}
            </div>
          </main>
        </div>
